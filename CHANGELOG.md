...
- **Pipe Input Compatibility**: Restore stdin pipe shortcuts for formatter
  - `cat data.json | jf` now works again
  - `cat data.json | jf --` now works again
  - Support `jf format -` to read JSON from stdin explicitly
...
- **Search Command**: Add `--full` flag to output full JSON value instead of preview
